...
if $magiskdata; then
  ...
  cp -af $tools/magisk* $tmp/magiskcommon/* $tools/busybox $tools/magiskboot $tools/magiskinit $tools/magiskpolicy   $MAGISKBIN
  chmod -R 755 $MAGISKBIN
  magiskinit -x magisk $MAGISKBIN/magisk
fi

# addon.d
if [ -d /system_root/system/addon.d ]; then
  ui_print "    Adding addon.d survival script"
  ADDOND=/system_root/system/addon.d/99-magisk.sh
...
"add 644 sbin/magiskpolicy $tools/magiskpolicy" \
"add 644 sbin/magisk $tools/magisk" \
"add 644 sbin/magiskboot $tools/magiskboot" \
"add 644 sbin/sgdisk $tools/sgdisk" \
"patch" \
